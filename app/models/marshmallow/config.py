...
from selenium_driver.settings import (
    DEFAULT_SCROLL_PAUSE_TIME,
    DEFAULT_REQUIRED_LOADING_DELAY,
    DEFAULT_REQUIRED_LOADING_TIMEOUT,
    DEFAULT_LOADING_DELAY,
    DEFAULT_LOADING_TIMEOUT,
    DEFAULT_SLEEP_AFTER_GET,
    DEFAULT_SCROLL_SLEEP,
    DEFAULT_SLEEP_AFTER_OPERATIONS,
    DEFAULT_CLOUDFLARE_BYPASS_TIME,
)
...
class SearchPageURLBuilderModuleSchema(CustomSchema):
    ...
    image_search_page_url_templates = fields.List(fields.Str())
...
class IdentifierRetrieverModuleSchema(CustomSchema):
    ...
    shadow_host_css_selector = fields.Raw(missing=None)
    ...
    force_javascript_click = fields.Boolean(missing=False)
    variants_css_selector = fields.Str(missing=None)
    picture_module_name = fields.Str()
    ...
    fixed_scroll_to = fields.Integer(missing=None)
    ...
    is_json_obj = fields.Boolean(default=False, missing=False)
    value = fields.Str(missing=None)
    restart_driver = fields.Boolean(missing=False)
    page_multiplier = fields.Integer(missing=1)
...
class SearchPageBrowsingModuleSchema(BasePostIdentifiersListModuleSchema):
    ...
    max_posts_to_browse = fields.Integer(default=100, missing=100)
...
class PostInformationRetrieverModuleSchema(BasePostIdentifiersListModuleSchema):
    ...
    use_light_post_info_only = fields.Boolean(default=False, missing=False)
...
class PosterInformationRetrieverModuleSchema(CustomSchema):
    ...
    followers_count_retriever_module: IdentifierRetrieverModuleSchema = fields.Nested(
        IdentifierRetrieverModuleSchema(), missing=None
    )
    description_retriever_module: IdentifierRetrieverModuleSchema = fields.Nested(
        IdentifierRetrieverModuleSchema(), missing=None
